...
public class ForceDotComOAuth2 extends Controller {
	...
	{
		OAuthSession sess = (OAuthSession)Cache.get(session.getId() + "-oauth");
		Logger.info("Session in Cache is:"+sess);
		return sess;
	}	
	
	public static boolean login( String clientId,
								String sec,
								OAuthListner list)
	{
	...
	public static boolean login(String authURL,
							  	String tokenURL,
							  	String clientId,
							  	String sec,
							  	OAuthListner list)
	{
		...
		if (sess == null)
		{
			if (isPersistentSession())
			{
				...
				{
					String id = userId.value.substring(0, userId.value.indexOf("-"));
					String sign = userId.value.substring(userId.value.indexOf("-") + 1);
	...
	{
		String persistent = Play.configuration.getProperty("sfdc.persistentSession");
	...
	private static void initiateOAuthFlow(OAuthListner list)
	{
		...
		throw new Redirect(authorizationURL
                ...
                + "&redirect_uri=" + WS.encode(play.mvc.Router.getFullUrl("ForceDotComOAuth2.callback")));
	}
	
	public static void callback()
	{
		Logger.info("callback called");
		OAuthListner listener = (OAuthListner)Cache.get(session.getId()+"-listener");
		if (listener == null)
			return;
		...
		if (accessCode == null)
		{
			listener.onFailure(Params.current().get("error"), Params.current().get("error_description"));
		}
		else
		{
			...
			params.put("redirect_uri", play.mvc.Router.getFullUrl("ForceDotComOAuth2.callback"));
			...
			if (accessToken != null)
			{
				...
				if (isPersistentSession())
				{
					Response.current().setCookie("uid", id + "-" + Crypto.sign(id), "30d");
					s.save();
				}
			...
			{
				listener.onFailure(r.getAsJsonPrimitive("error").getAsString(), r.getAsJsonPrimitive("error_description").getAsString());
			}
		}
	}

	public static boolean refreshToken(String tokenURL,
									String clientId,
									String sec)
	{
		if (!isLoggedIn())
		...
		if (accessToken != null)
		{
			...
			String id = currentSession.idURL.substring( currentSession.idURL.lastIndexOf('/') + 1);
